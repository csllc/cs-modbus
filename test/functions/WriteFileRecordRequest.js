...
describe("WriteFileRecordRequest", function()
{
  it("should throw if the specified fileNumber is invalid", function()
  {
    function testZero()
    {
      new WriteFileRecordRequest([{fileNumber: 0, recordData: Buffer.alloc(2)}]);
    }

    function testLessThanZero()
    {
      new WriteFileRecordRequest([{fileNumber: -1337, recordData: Buffer.alloc(2)}]);
    }

    function testGreaterThanFFFF()
    {
      new WriteFileRecordRequest([{fileNumber: 0xFFFF + 1, recordData: Buffer.alloc(2)}]);
    }
  ...
  {
    new WriteFileRecordRequest([{recordData: Buffer.alloc(2)}]).getSubRequests()[0].fileNumber.should.be.equal(0x0001);
  });

  it("should use 0x0000 as a default recordNumber", function()
  {
    new WriteFileRecordRequest([{recordData: Buffer.alloc(2)}]).getSubRequests()[0].recordNumber.should.be.equal(0x0000);
  });
  ...
  describe("getCode", function()
  {
    ...
    {
      new WriteFileRecordRequest([{recordData: Buffer.alloc(2)}]).getCode().should.be.equal(0x15);
    });
  });
  ...
  describe("fromBuffer", function()
  {
    it("should throw if the specified Buffer is not at least 11 bytes long", function()
    {
      function test1()
      {
        WriteFileRecordRequest.fromBuffer(Buffer.from([]));
      }
    ...
    it("should throw if the first byte is an invalid function code", function()
    {
      function test()
      {
        WriteFileRecordRequest.fromBuffer(Buffer.from([0x14, 0x09, 0x06, 0x00, 0x01, 0x00, 0x00, 0x00, 0x01, 0xab, 0xcd]));
      }
    ...
    it("should throw if the reference type byte is not equal to 6", function()
    {
      function test()
      {
        WriteFileRecordRequest.fromBuffer(Buffer.from([0x15, 0x09, 0xFF, 0x00, 0x01, 0x00, 0x00, 0x00, 0x01, 0xab, 0xcd]));
      }
  ...
  describe("toBuffer", function()
  {
    ...
    {
      new WriteFileRecordRequest([{recordData: Buffer.from([0x11, 0x11])}]).toBuffer()[0].should.be.equal(0x15);
    });
  ...
  describe("toString", function()
  {
    ...
    {
      new WriteFileRecordRequest([{recordData: Buffer.from([0x11, 0x11])}]).toString().should.be.a.String();
    });
  });
  ...
  describe("createResponse", function()
  {
    ...
    {
      var req = new WriteFileRecordRequest([{recordData: Buffer.from([0x11, 0x11])}]);
      var res = req.createResponse(Buffer.from([0x95, 0x02]));
    ...
    it("should return an instance of WriteFileRecordResponse if the function code is not an exception", function()
    {
      ...
      var res = req.createResponse(Buffer.from([
        0x15, 0x12,
        0x06, 0x00, 0x01, 0x00, 0x00, 0x00, 0x01, 0x11, 0x11,
        0x06, 0x00, 0x02, 0x00, 0x04, 0x00, 0x01, 0x22, 0x22
      ]));
