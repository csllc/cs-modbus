...
SocketcandTransport.prototype.buildAduReadMemory = function(transaction) {
  ...
  if (bank >= 0 && bank <= 3) {
    ...
    command = 0x49; // comm processor memory access
    opcode = 0x43; // read command
  }
  ...
  for (var i = 0; i < request.count; i++) {
    ...
    chunks.push({
      id: MESSAGE_ID + (transaction.getUnit() << 8) + this.myCanId,
      ext: true,
      buf: Buffer.from([command, opcode, address + i])
...
SocketcandTransport.prototype.onData = function(data) {
  ...
  if (data.ext && (pf === 0x00EF0000) && (to === this.myCanId)) {
    ...
    console.log(data.id.toString(16), data.buf);
    ...
    if (this.transactions[from] &&
      ...
      console.log(chunk, from, cmd, op, address, value, response.length);

      // special case for reading comm eeprom
      if (cmd === 0x49 && op === 0x43 && !Array.isArray(data)) {
