...
CS1179Transport.prototype.buildAduWriteMemory = function(transaction) {
  ...
  for (var i = 0; i < values.length; i++) {
    ...
    switch (bank) {
      ...
      // case 2 - Shadow Write
      case 2:
        // Assuming address was sent to this function with bank added (writeMemory() does this)
        // SHEE bank is 512, subtract to get base address.
        address = address - 512; 
        console.log("CS1179Transport :: Address = 0x"+ address.toString(16))
        ...
        offset = address & 0xFF; 
        console.log("CS1179Transport :: Corrected address = 0x" + address.toString(16))

         // Determine the opcode based on the returned address
         // Lo Ram Write
        if (address >= 0x0A0 && address <= 0x0EF) {
          console.log("CS1179Transport :: Address 0x" + address.toString(16), "implies Low Ram Write opcode");
          opcode = USB_I2C_WRITE_LO_RAM;
        // Hi Ram Write
        } else if ((address >= 0x190 && address <= 0x1AF) ||
                  (address >= 0x1B0 && address <= 0x1CF) ||
                  (address >= 0x130 && address <= 0x15F)) {
          console.log("CS1179Transport :: Address 0x" + address.toString(16), "implies Hi Ram Write opcode");
          opcode = USB_I2C_WRITE_HI_RAM;
        } else {
    ...
    console.log("Pushing opcode", opcode, "offset", offset, "values[i]", values[i], "to chunks")
  }
...
CS1179Transport.prototype.finishResponse = function() {
  ...
  try {
    ...
    console.log("CS1179 transaction:", transaction);
  }
  catch (error) {
