...
CS1179Transport.prototype.finishResponse = function() {
  ...
  try {
    ...
    if( request.code === MB_READ_MEMORY ) {
      resp = request.createResponse( new Buffer( this.reader )) ;
      ...
      resp = request.createResponse( new Buffer( [MB_WRITE_MEMORY, 0])) ;
      
      this.clearTransaction();
      transaction.handleResponse(request.createResponse( resp ));

    }
    else if( request.code === MB_SLAVEID ) {
      resp = request.createResponse( new Buffer( [
        MB_SLAVEID,
        9,    // length
        this.reader[1],
        255,
