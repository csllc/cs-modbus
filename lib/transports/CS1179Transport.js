...
const MB_READ_MEMORY = 0x45;
const MB_WRITE_MEMORY = 0x46;
const MB_WRITE_MEMORY_VERIFY = 0x64;
const MB_SLAVEID = 0x11;
const MB_COMMAND = 0x47;
const MB_RESET = 0xFB;

// Phoenix restart function
const PHOENIX_RESTART_ADDR = 0x1E0;
const PHOENIX_RESTART_1ST_BYTE = 0xDE;
const PHOENIX_RESTART_2ND_BYTE = 0xAC;
 ...
 */
const RAM_WR_CMD = 0x00 // -> 0x00
const HRAM_WR_CMD = 0x20 // -> 0x06
const ARAM_WR_CMD = 0x40 // -> 0x04
const RAM_RD_CMD = 0x01 // -> 0x01
const HRAM_RD_CMD = 0x21 // -> 0x07
const ARAM_RD_CMD = 0x41 // -> 0x05
const E2_WR_CMD = 0x06 // -> 0xB0
const E2_RD_CMD = 0x07 // -> 0xA0
const E2_WRVFY_CMD = 0x0B // -> 0xB0, 0xA0
const INVALID = 0xFF;

const USB_I2C_RESET = 0x0F;
  ...
  for (var i = 0; i < values.length; i++) {
    ...
    switch (bank) {
      ...
      case 2:
        ...
        case 0x300:
          // EEPROM Write
          // Writes to SHEE backed by EE (and not RAM) fail silently
          // in the original Phoenix CAN software. To emulate that,
          // elicit an error resposne from the CS1179.
          opcode = INVALID;
          opcodeToString = 'invalid';
          break;
        default:
          throw new Error(`Translated address 0x${address.toString(16)} is out of range.`);
        }
 ...
 * @returns {Buffer}
 */
CS1179Transport.prototype.buildAduCommand = function(transaction) {
  ...
  switch (request.id) {
    case MB_RESET:
      ...
      writeTransaction.request = {address: PHOENIX_RESTART_ADDR, values: [PHOENIX_RESTART_1ST_BYTE]};
      chunks.push(this.buildAduWriteMemory(writeTransaction)[0]);

      writeTransaction.request = {address: PHOENIX_RESTART_ADDR, values: [PHOENIX_RESTART_2ND_BYTE]};
      chunks.push(this.buildAduWriteMemory(writeTransaction)[0]);
      break;
    default:
      // Original behavior retained for commands we don't know about
      chunks.push([USB_I2C_RESET, 0, 0]);
      break;
  }
