...
var codeToMessageMap = {
  ...
  0x80: 'Verify Error: Slave was unable to verify memory was written '
    + 'correctly.',
};
...
function ExceptionResponse(functionCode, exceptionCode)
{
  ...
  /**
   * A code of the function that resulted in the exception.
   *
   * @private
   * @type {number}
   */
...
{
  return Buffer.from([this.getCode() + 0x80, this.exceptionCode]);
};
...
ExceptionResponse.prototype.toString = function ()
{
  ...
  if (this.functionCode < 0xF)
  {
    functionCode += '0';
  }

  functionCode += this.functionCode.toString(16);

  // Build exception code + message string
